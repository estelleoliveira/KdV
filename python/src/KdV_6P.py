...
Projet_simulation_numerique : KDV modèle 6P
...
def sixP_schema(L, T, M, N, eta, mu, x0, Uinf, U0):
...
# ----------------------------------------------------------------
    ...
    print("r6P  numérique ", r)
    print("r6P  théorique max  ",rtheo)
...
# Calcul de u[j,n+1], on pose D,E,F pour plus de visibilité
            D = -3*alpha*(u[jp, n] + u[j, n])*(u[jp, n] - u[j, n])
            E = -2*S*(u[jw, n] - (3*u[jp, n]) + (3*u[j, n]) - u[jm, n])
            F = - u[jp, n] + u [j, n] + u[jp, n-1]
            u[j, n+1] = D + E + F
            
            umax=np.max(np.abs(u))
        if umax>1e6:    
            print('Instabilité numérique sixP: boum au pas '+str(n))
            break
        if (n+1)%100==0:
            print('Pas '+str(n+1)+' sur '+str(N))
        if (n+1)==N:
            print('Dernier pas atteint sixP')

    print('temps de calcul modèle 6P = ', time.ctime(time.time() - start)[13:19],' minutes')
    return u